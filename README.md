...
У середовищі візуального програмування node-red створено фрагмент коду для зчитування конфігурацій системи та контролера. Функція SystemState створює об'єкт у глобальному контексті з усіма даними про систему. Функція getConfig створює массив у глобальному контексті з конфігураціями контролера які можна зберігати у першому регістрі (Рис.2). Кожен елемент масиву відповідає порядкому значенню у регістрі. Приклад реалізації даної функції (див Рис.3).
...
Рисунок 3. Реалізація функції getConfig
...
Для того, щоб записувати real-time дані у БД використаємо алгоритм, що складається з блоків для зчитування даних з flow змінної, форматування даних у JSON та запиус в базу даних:
...
У ноді toDbFormat визначаємо форматування вхідних даних у JSON який будемо передавати в базу даних:
